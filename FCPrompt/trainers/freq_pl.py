...
def load_clip_to_cpu(cfg):
    ...
    design_details = {
                    "prompt_depth": cfg.TRAINER.PLOT.DEPTH,
                    "language_ctx": cfg.TRAINER.PLOT.N_CTX,
                    "vision_ctx": cfg.TRAINER.PLOT.V_CTX,
                    "n_textprompt": cfg.TRAINER.PLOT.N,
                    "n_selected": cfg.TRAINER.PLOT.SELECTED
                    }
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            ctx_vectors_0 = embedding[0, 1 : 1 + n_ctx, :].unsqueeze(0)
            ctx_vectors_1 = torch.empty(self.N, n_ctx, ctx_dim, dtype=dtype) 
            nn.init.normal_(ctx_vectors_1, std=0.02) 
            ctx_vectors = torch.cat([ctx_vectors_0, ctx_vectors_1], dim=0)
            ...
            if cfg.TRAINER.PLOT.CSC:
                ...
                ctx_vectors = torch.empty(self.N+1, n_ctx, ctx_dim, dtype=dtype) 
        ...
        tokenized_prompts = tokenized_prompts.repeat(self.N+1,1) 
    ...
    def forward(self):
        ...
        ctx = ctx.contiguous().view((self.N+1)*self.n_cls,self.n_ctx,ctx.shape[3])
        # print(ctx.shape)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        b = image.shape[0]
        image_feature, freq_features, mse_loss = self.image_encoder(image.type(self.dtype))  
        image_features = freq_features.permute(1, 0, 2)
        image_feature_pool = image_feature
        # image_features = freq_features
        ...
        if self.dataset == "ImageNet":
            ...
            text_features_all =  text_features.contiguous().view(self.N+1, self.n_cls, self.d)  
            text_feature_pool = text_features_all[0,:,:]
            text_features = text_features_all[1:,:,:]
        else:
        ...
        # print(text_features.sum(dim=2))
        # print(text_feature_pool.shape)
        text_feature_pool = F.normalize(text_feature_pool, dim=1)

        # text_a = text_features
        # # text_a = text_features.view(text_features.shape[0]*text_features.shape[1], -1)
        # text_b = text_a.permute(0, 2, 1)
        # # text_features_T = text_features.permute(1, 0, 2)
        # similarity_score = torch.matmul(text_a, text_b)
        # target_score = torch.eye(text_a.shape[1], dtype=similarity_score.dtype, device=similarity_score.device)
        # # print(target_score[0:10, 0:10])
        # target_score = target_score.expand(text_a.shape[0], -1 , -1)
        # # print(target_score.shape)
        # mse_loss = self.mse_loss(similarity_score, target_score)
        ...
        return logits, logits2, mse_loss


@TRAINER_REGISTRY.register()
class FREQPL(TrainerX):
    """
    It is based on CoOp.
    """
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output, output2, mse_loss = self.model(image)
            loss1 = F.cross_entropy(output, label)
            loss2 = F.cross_entropy(output2, label)
            loss = loss1 + loss2 + mse_loss*0.1
